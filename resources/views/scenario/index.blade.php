...
@section('content')
            ...
            <li class="col-span-1 flex flex-col divide-y divide-white/10 rounded-lg bg-gray-800/50 text-center outline -outline-offset-1 outline-white/10">
                ...
                <div>
                    <div class="-mt-px flex divide-x divide-white/10">
                        <form action="{{ route('scenarios.destroy', $scenario) }}" method="post" class="flex w-0 flex-1">
                            @csrf
                            @method('delete')
                            <button class="relative cursor-pointer -mr-px inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-bl-lg border border-transparent py-4 text-sm font-semibold text-white">
                        ...
                        <div class="-ml-px flex w-0 flex-1">
                            <a href="{{ route('scenarios.show', ['scenario' => $scenario]) }}" class="relative inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-br-lg border border-transparent py-4 text-sm font-semibold text-white">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-5 text-gray-500">
    ...
    @if($scenarios->isEmpty())
        <div class="my-8">
            <p class="text-center text-white">No scenario found.</p>
        </div>
    @endif
