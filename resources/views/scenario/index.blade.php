                ...
                <div>
                    <div class="-mt-px flex divide-x divide-white/10">
                        <form action="{{ route('scenarios.destroy', $scenario) }}" method="post" class="flex w-0 flex-1">
                            @csrf
                            @method('delete')
                            <button class="relative -mr-px inline-flex w-0 flex-1 items-center justify-center gap-x-3 rounded-bl-lg border border-transparent py-4 text-sm font-semibold text-white">
